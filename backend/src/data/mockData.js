export const initialProjects = [
  {
    id: 1,
    titre: "DZ TECH STORE — Magasin & Catalogue E-Commerce High-Tech",
    description: "Plateforme Web e-commerce et vitrine haut de gamme spécialisée dans la vente d'équipements High-Tech (Smartphones neufs sous emballage, téléphones Caba certifiés, et PC portables Gamer / Pro). Interface moderne avec bannières promotionnelles, sélection par gamme et livraison sécurisée 58 Wilayas.",
    image_url: "/assets/dztechstore_hero.png",
    images: ["/assets/dztechstore_hero.png", "/assets/dztechstore_catalog.png"],
    technologies: ["React.js", "Node.js / Express", "Catalogue High-Tech", "E-Commerce", "Livraison 58 Wilayas", "Design Premium"],
    lien_projet: "https://github.com/salaheddinesaidi01-bot/portfolio-",
    lien_code: "https://github.com/salaheddinesaidi01-bot/portfolio-",
    date_creation: new Date("2026-02-01")
  },
  {
    id: 2,
    titre: "Site web pour boutique de vêtements destiné au e-commerce",
    description: "Site web e-commerce moderne et responsive pour boutique de vêtements et prêt-à-porter (KOUNOUZ). Interface élégante avec univers Homme, Femme, Enfant, catalogue dynamique, filtres et expérience d'achat fluide.",
    image_url: "/assets/project_ecommerce.png",
    technologies: ["HTML5/CSS3", "JavaScript", "E-Commerce", "Design Responsive", "Mode & Vêtements"],
    lien_projet: "https://f5db1c4k-5174.euw.devtunnels.ms/",
    lien_code: "https://github.com/salaheddinesaidi01-bot/portfolio-",
    date_creation: new Date("2026-01-15")
  },
  {
    id: 3,
    titre: "Tableau de Bord & Dashboard Décisionnel d'Entreprise",
    description: "Application web d'aide à la décision et de pilotage d'entreprise. Génération automatique de rapports (Direction, Commercial), suivi du chiffre d'affaires, gestion de la flotte logistique, livreurs et seuil de rentabilité.",
    image_url: "/assets/project_dashboard.png",
    technologies: ["HTML5/CSS3", "JavaScript", "Aide à la Décision", "Rapports Automatiques", "Dashboard Analytics"],
    lien_projet: "https://github.com/salaheddinesaidi01-bot/portfolio-",
    lien_code: "https://github.com/salaheddinesaidi01-bot/portfolio-",
    date_creation: new Date("2025-11-20")
  }
];

export const initialAboutInfo = {
  id: 1,
  nom: "SAIDI SALAH EDDINE",
  titre: "Développeur Web | Data Analyst | Ingénieur Génie Industriel",
  presentation: "Passionné par le développement web, l'analyse de données et l'ingénierie industrielle. Je conçois des applications web modernes, des solutions SaaS et des tableaux de bord sur-mesure pour optimiser les performances opérationnelles et la prise de décision.",
  parcours: "Ingénieur en Génie Industriel spécialisé en optimisation des flux et logistique, Data & Business Analyst, et Développeur Web spécialisé dans la conception de dashboards décisionnels et de plateformes SaaS.",
  skills: [
    { category: "Web & SaaS", name: "Développement de Sites Web (Vitrines & E-Commerce)", level: 95, icon: "Code2" },
    { category: "Web & SaaS", name: "Conception de Tableaux de Bord (Dashboards)", level: 95, icon: "BarChart3" },
    { category: "Web & SaaS", name: "Développement SaaS & Web Apps", level: 92, icon: "Layout" },
    { category: "Frontend", name: "HTML5 / CSS3 / JavaScript Modern", level: 90, icon: "FileCode" },
    { category: "Frontend", name: "React.js & Intégration UI/UX", level: 92, icon: "Palette" },
    { category: "Data & Ingénierie", name: "Data & Business Analysis", level: 92, icon: "LineChart" },
    { category: "Data & Ingénierie", name: "Génie Industriel & Optimisation des Flux", level: 95, icon: "Cpu" },
    { category: "Logistique", name: "Gestion de la Logistique & Supply Chain", level: 90, icon: "Box" },
    { category: "Outils", name: "Git & Versioning", level: 90, icon: "GitBranch" }
  ],
  timeline: [
    {
      year: "Récents",
      title: "Développeur Web & Concepteur SaaS / Dashboards",
      company: "Création de Applications Web & Dashboards Sur-Mesure",
      description: "Conception et développement de sites e-commerce performants, d'applications SaaS et de tableaux de bord décisionnels interactifs."
    },
    {
      year: "Expérience",
      title: "Data & Business Analyst",
      company: "Analyse de Données & Aide à la Décision",
      description: "Analyse de données stratégiques, modélisation d'indicateurs clés de performance (KPIs), création de dashboards décisionnels et automatisation de rapports."
    },
    {
      year: "Formation & Parcours",
      title: "Ingénieur en Génie Industriel",
      company: "Spécialité Optimisation des Flux & Logistique",
      description: "Modélisation et optimisation de la chaîne logistique, gestion des stocks, cartographie des flux et amélioration continue de la chaîne de valeur."
    }
  ]
};


